...
$stm=" AND bookingprice BETWEEN ".$_GET["min_price"]."  AND ".$_GET["max_price"]." AND carpetarea>=".$_GET["lot_sqft"]." AND possession>=".$_GET["year_built"]." AND apartmenttype>=".$_GET["bedrooms"];


$temp = "&min_price=".$_GET["min_price"]."&max_price=".$_GET["max_price"]."&lot_sqft=".$_GET["lot_sqft"]."&year_built=".$_GET["year_built"]."&bedrooms=".$_GET["bedrooms"];
$ref = $ref.$temp;
...
{
	$stm= " AND id IN (SELECT distinct d.id FROM estatefeaturejoin d WHERE ";
	$sql=$sql.$stm;
	$i=0;
 	...
 	foreach ($_GET['feature'] as $feature)
	{
		if($j<$i)
		{
        	$stm = "d.id  in (SELECT id FROM estatefeaturejoin WHERE featureid = $feature) AND ";
	...
	$stm="d.id  in (SELECT id FROM estatefeaturejoin WHERE featureid = $feature))";
	$temp = "&feature[]=".$feature;
	$ref = $ref.$temp;
	$sql=$sql.$stm;
...
for ($i=1; $i<=$total_pages; $i++) {  
             $pagLink .= "<li><a href='estatelist.php?page=".$i.$ref."'>".$i."</a></li>";  
};  
echo $pagLink . "</ul></nav>";  
